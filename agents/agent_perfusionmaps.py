...
from graphs.losses.losses import create_loss_module
from graphs.losses.losses_w import loss_module

from graphs.models.perfusionmodel import MapsNet
from utils.metrics import DiceMetric
from utils.train_utils import adjust_learning_rate, adjust_alpha
from torch.optim.lr_scheduler import StepLR

from torchsummary import summary
from datasets.dataloader_perfusionmaps import PerfusionDataLoader
import matplotlib.pyplot as plt
from utils.metrics import AverageMeter
from utils.valid_utils import SaveProbMap, getMetrics, getMetricsFindThreshold
import wandb
import glob
# first pycharm commit
...
class AgentPerfusion:
    def __init__(self):
        ...
        if self.cuda:
            ...
            self.logger.info("Operation will be on *****GPU-{}***** ".format(self.device))
        else:
            self.device = torch.device("cpu")
            self.logger.info("Operation will be on *****CPU***** ")
        # Create an instance from the Model
        if self.config.modelname == 'MapsNet':
            self.model = MapsNet(self.config)

        print(summary(self.model, input_size=(self.config.input_channels*5, 256, 256), device='cpu'))

        self.model = self.model.float().to(self.device)

        self.nonlin = torch.nn.Softmax(dim=1)
        # Create an instance from the data loader
        self.data_loader = PerfusionDataLoader(self.config)
        # loss function
    ...
    def train_one_epoch(self):
        ...
        for case_name, inputs, masks in tqdm_batch:
            ...
            inputs = rearrange(inputs, 'b h w c -> b c h w').to(self.device)
    ...
    def validate_3d(self, final=False):
        ...
        if self.config.optimize_th:
            metrics = getMetricsFindThreshold(self.config, wandb.run.dir,
